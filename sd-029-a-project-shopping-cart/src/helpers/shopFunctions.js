...
export const calcCartPrice = () => {
  ...
  const productsPriceValue = cartProducts
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  const cartProducts = document
    .querySelector('.cart__products');
  //
  const addProductsIntoCart = async () => {
    const cartId = cartButton
      .parentElement.firstChild.innerText;
    saveCartID(cartId); //
    const productInfo = await fetchProduct(cartId);//
    //
    const selectedProduct = createCartProductElement(productInfo);//
    cartProducts
      .appendChild(selectedProduct);//
    calcCartPrice();
  };

  cartButton.addEventListener('click', addProductsIntoCart);
