...
// let newConsumption = [];
const createMenu = (objectForParameter) => {
  const meuMenu = {
  fetchMenu: () => objectForParameter,
  ...
  order: (string) => {
    let foods = Object.keys(objectForParameter.food);
    let drinks = Object.keys(objectForParameter.drink);
    if (
      foods.includes(string) || drinks.includes(string)
    ) {
      meuMenu.consumption.push(string);
    } else {
  ...
  pay: () => {
    let bill = 0;
    meuMenu.consumption.forEach((string) => {
      if (Object.keys(objectForParameter.food).includes(string)) {
        bill += objectForParameter.food[string];
      }
      if (Object.keys(objectForParameter.drink).includes(string)) {
        bill += objectForParameter.drink[string];
      }
    });
    return bill * 1.1;
  },
