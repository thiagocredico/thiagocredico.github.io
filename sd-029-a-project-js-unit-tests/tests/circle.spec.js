...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    const testOfCircleFunction = circle(3);
    // ESCREVA SEUS TESTES ABAIXO:
    // Teste se circle retorna undefined, caso o parâmetro passado não seja um número.
    expect(circle('NaN')).toBeUndefined();
    // Teste se circle retorna um objeto.
    expect(typeof testOfCircleFunction).toBe('object');
    // Teste se o objeto retornado possui 3 propriedades.
    expect(Object.keys(testOfCircleFunction).length).toBe(3);
    ...
    testOfCircleFunction.area = parseFloat((testOfCircleFunction.area).toPrecision(4));
    expect(testOfCircleFunction).toEqual({radius: 3, area: 28.26, circumference: 18.84});
  });
});
